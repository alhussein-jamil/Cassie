...
def randomize_model():
        ...
        for joint in body.iter("joint"):
            ...
            if joint.get("damping") != None:
                joint.set(
                    "damping",
                    str(
                        float(joint.get("damping"))
                        * np.random.uniform(
                            ranges["joint_damping"][0], ranges["joint_damping"][1]
                        )
                    ),
                )
        ...
        for bodypart in body.iter("body"):
            if bodypart.get("name") == "cassie-pelvis":
                ...
                bodypart.set(
                    ...
                    str(
                        np.random.uniform(
                            ranges["pelvis_com_x"][0], ranges["pelvis_com_x"][1]
                        )
                    )
                    + " "
                    + str(
                        float(pelvis_pos[1])
                        + np.random.uniform(
                            ranges["pelvis_com_y"][0], ranges["pelvis_com_y"][1]
                        )
                    )
                    + " "
                    + str(
                        float(pelvis_pos[2])
                        + np.random.uniform(
                            ranges["pelvis_com_z"][0], ranges["pelvis_com_z"][1]
                        )
                    ),
                )
    ...
    tree.write(
        "/home/alhussein.jamil/.pyenv/versions/3.8.16/envs/sandbox/lib/python3.8/site-packages/gymnasium/envs/mujoco/assets/cassie_randomized.xml"
    )
...
class CAPSModel(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.condition_space = nn.Parameter(torch.ones((1, 1)))
    ...
    def forward(self, input_dict, state, seq_lens):
        obs = input_dict["obs"]
        condition = self.condition_space.expand(obs.shape[0], 1)
        logits = self.fc_net(obs) * torch.sqrt(condition)
        return logits, []
...
def loss_fn(policy, model, dist_class, train_batch):
    logits, _ = model(train_batch["obs"])
    action_dist = dist_class(logits, model)

    # Compute the CAPS loss
    action_mean = action_dist.sample().mean(dim=0, keepdim=True)
    caps_loss = ((model.condition_space - action_mean) ** 2).mean()

    # Compute the PPO loss
    ppo_loss = policy._compute_ppo_loss(model, dist_class, train_batch)

    # Combine the two losses
    loss = ppo_loss + policy.config["caps_coeff"] * caps_loss
    return loss
