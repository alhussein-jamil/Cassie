...
class CassieEnv(MujocoEnv):
    ...
    def _get_obs(self):
        ...
        p = np.array(
            [
                np.sin((2 * np.pi * (self.phi + THETA_LEFT)) / STEPS_IN_CYCLE),
                np.sin((2 * np.pi * (self.phi + THETA_RIGHT)) / STEPS_IN_CYCLE),
            ]
        )
    ...
    def compute_reward(self, action):
        ...
        q_left_frc = 1.0 - np.exp(
            -OMEGA * np.linalg.norm(contact_force_left_foot) ** 2 / 4000
        )
        q_right_frc = 1.0 - np.exp(
            -OMEGA * np.linalg.norm(contact_force_right_foot) ** 2 / 4000
        )
        ...
        I = lambda phi, a, b: self.vm_cdf(phi, a, KAPPA) * (
            1 - self.vm_cdf(phi, b, KAPPA)
        )
        ...
        C_frc = (
            lambda phi: c_swing_frc * I_swing_frc(phi)
        ...
        self.rewards = {
            ...
            "reward": reward,
            "contact_force_left_foot": np.linalg.norm(contact_force_left_foot) ** 2
            / 4000,
        }
        return reward
