...
class SnakeEnv(gym.Env):
    ...
    def compute_reward(self, action):
        ...
        if (
            self.snake.head[0] < 0
            or self.snake.head[0] >= self.screen_width
            or self.snake.head[1] < 0
            or self.snake.head[1] >= self.screen_height
        ):
            self.reward -= 1
        ...
        self.reward += np.exp(
            -3
            * np.sqrt(
                ((self.snake.head[0] - self.apple.position[0]) / self.block_size) ** 2
                + ((self.snake.head[1] - self.apple.position[1]) / self.block_size) ** 2
            )
            / (
                (
                    self.screen_width / self.block_size
    ...
    def generate_apple(self):
        ...
        x = (
            random.randint(0, (self.screen_width - self.block_size) // self.block_size)
            * self.block_size
        )
        y = (
            random.randint(0, (self.screen_height - self.block_size) // self.block_size)
            * self.block_size
        )
        ...
        while (x, y) in self.snake.body or (x, y) == self.snake.head:
            x = (
                random.randint(
                    0, (self.screen_width - self.block_size) // self.block_size
                )
                * self.block_size
            )
            y = (
                random.randint(
                    0, (self.screen_height - self.block_size) // self.block_size
                )
                * self.block_size
            )
    ...
    def render(self, mode="rgb_array"):
        if mode == "rgb_array":
            ...
            # red for the apple
            image[
                self.apple.position[1] : self.apple.position[1] + self.block_size,
                self.apple.position[0] : self.apple.position[0] + self.block_size,
                :,
            ] = [255, 0, 0]
            ...
            for pos in self.snake.body:
                image[
                    pos[1] : pos[1] + self.block_size,
                    pos[0] : pos[0] + self.block_size,
                    :,
                ] = [0, 255, 0]
            # blue for the head
            image[
                self.snake.head[1] : self.snake.head[1] + self.block_size,
                self.snake.head[0] : self.snake.head[0] + self.block_size,
                :,
            ] = [0, 0, 255]
            return image
        else:
