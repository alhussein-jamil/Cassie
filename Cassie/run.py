...
if __name__ == "__main__":
    ...
    if(is_dict):
        if(not old_implementation):
            ...
            trainer = Trainer(config=config,env="cassie-v0", logger_creator=lambda config: UnifiedLogger(config, log_dir))
    ...
    while True:
            ...
            i += 1
            print("Episode {} Reward Mean {} Q_lef_frc {} Q_left_spd {}".format(i, result["episode_reward_mean"], result['custom_metrics']['custom_quantities_q_left_frc_mean'] , result['custom_metrics']['custom_quantities_q_left_spd_mean']))
